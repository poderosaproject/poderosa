...
namespace Poderosa.Util {
    ...
    public class EnumDescAttribute : Attribute {
        ...
        private Type _type;
        private Type _enumListItemType;
        ...
        public void Init(Type t) {
            _type = t;
            _enumListItemType = typeof(EnumListItem<>).MakeGenericType(_type);
        }
        ...
        public virtual string GetDescription(ValueType i) {
            ...
            object item = _enumListItemType.GetMethod("CreateListItem").Invoke(null, new object[] { i });
        ...
        public virtual ValueType FromDescription(string v, ValueType d) {
            if (d.GetType() != _type)
                throw new ArgumentException("Type unmatch", "d");

            if (v == null)
                return d;

            Array items = (Array)_enumListItemType.GetMethod("GetListItems").Invoke(null, null);
            foreach (object item in items) {
                if (v == item.ToString()) {
                    return (ValueType)item.GetType().GetProperty("Value").GetValue(item, null);
                }
            }
        ...
        public virtual ValueType FromName(string v) {
            return (ValueType)Enum.Parse(_type, v);
        }
        ...
        public virtual string[] DescriptionCollection() {
            ...
            for (int i = 0; i < items.Length; i++)
                texts[i] = items.GetValue(i).ToString();
            return texts;
        }

        [Obsolete]
        public static EnumDescAttribute For(Type type) {
            return new EnumDescAttribute(type);
        }
