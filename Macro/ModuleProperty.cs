 ...
 *
 * $Id: ModuleProperty.cs,v 1.3 2011/10/27 23:21:56 kzmi Exp $
 */
using System;
using System.IO;
...
namespace Poderosa.MacroInternal {
    ...
    internal class ModuleProperty : System.Windows.Forms.Form {
        ...
        private void OnSelectFile(object sender, EventArgs args) {
            string path = MacroPlugin.Instance.SelectMacro(this);
            if (path != null) {
                _path.Text = path;
                if (_title.Text.Length == 0)
                    _title.Text = System.IO.Path.GetFileName(path); //ファイル名本体をデフォルトのタイトルにする
            }
        }
